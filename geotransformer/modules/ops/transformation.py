...
def apply_transform(points: torch.Tensor, transform: torch.Tensor, normals: Optional[torch.Tensor] = None):
    ...
    # ★ (N,3) + (1,4,4) 또는 (B,N,3) + (1,4,4) 케이스 지원: 하나의 T를 모든 포인트/배치에 적용
    if transform.ndim == 3 and transform.shape[0] == 1:
        transform = transform[0]  # (4,4)로 변환

    if transform.ndim == 2:
        # transform: (4,4), points: (*,3) or (B,N,3) 모두 지원(전체에 동일 변환)
        rotation = transform[:3, :3]
        translation = transform[:3, 3]
        pts_shape = points.shape
        pts = points.reshape(-1, 3)
        pts = pts @ rotation.transpose(-1, -2) + translation
        points = pts.reshape(*pts_shape)
        if normals is not None:
            nrm = normals.reshape(-1, 3)
            nrm = nrm @ rotation.transpose(-1, -2)
            normals = nrm.reshape(*pts_shape)
        ...
        if normals is not None:
            normals = normals @ rotation.transpose(-1, -2)
